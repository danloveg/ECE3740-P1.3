...
#endif
{
	...
	// on startup
    //KF: start if(BUTTON0_IO == 0u) //KF:end
    if(BUTTON0_IO == BUTTON_DOWN)
    {
		...
		//KF: start while(BUTTON0_IO == 0u) //KF:end
		while(BUTTON0_IO == BUTTON_DOWN)
		{
...
static void InitializeBoard(void)
{
	...
	PORTSetPinsDigitalOut(IOPORT_B, BIT_11);	// Configure the pin connected to RXER/PHYAD0 as output
	PORTClearBits(IOPORT_B, BIT_11);	    		// Establish the physical address = 0

	PORTSetPinsDigitalOut(IOPORT_A, BIT_6); 	// Configure the pin connected to the 8720 Reset
	PORTClearBits(IOPORT_A, BIT_6);						// Place the 8720 in reset.
	DelayMs(1); 															// Delay for 1 ms: datasheet specifies at least 100 us
	PORTSetBits(IOPORT_A, BIT_6);					  	// Take the 8720 out of reset.
	// KF: End

// KF: start
	LED0_IO = 0; LED1_IO = 0; LED2_IO = 0; LED3_IO = 0;
	LED0_TRIS = 0; LED1_TRIS = 0;	LED2_TRIS = 0; LED3_TRIS = 0;
	LED0_IO = 1; LED1_IO = 1; LED2_IO = 1; LED3_IO = 1;
	DelayMs(10);
	LED0_IO = 0; LED1_IO = 0; LED2_IO = 0; LED3_IO = 0;
